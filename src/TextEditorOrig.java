 ...
 */
public class TextEditorOrig extends JPanel {	
	//private JFrame frame__;
	private JPanel frame__;
	...
	void createAndShowGUI() {
		...
		editor__ = new JTextPane(); //All of the editor is contained within this JTextPane
		JScrollPane editorScrollPane = new JScrollPane(editor__); //Wrap the editor in a JScrollPane
		...
		//frame__.setJMenuBar(menuBar); //GO BACK TO THIS!
	...
	//============== MY CUSTOM ACTION LISTENERS ==============
	private class PrintToConsoleActionListener implements ActionListener {
	...
	private class ReadFileActionListener implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
						
			//For the Handbook, first read contents from database and put those bytes into a file that we'd then read from here...?
			//If no Internet connection, then just read from the local file
			
			//readFile(new File("src/test")); //HARD-CODING
			readFile(new BufferedInputStream(
					TextEditorOrig.class.getResourceAsStream("test"))); //HARD-CODING

		}
		
		
		//private void readFile(File file) {
		private void readFile(BufferedInputStream file) {
			
			StyledDocument doc = null;
	
			//try (InputStream fis = new FileInputStream(file); ObjectInputStream ois = new ObjectInputStream(fis)) {
			try (BufferedInputStream fis = file; ObjectInputStream ois = new ObjectInputStream(fis)) {
		...
		public void actionPerformed(ActionEvent e) {	
	        ...
	        try {
	            BufferedReader br = new BufferedReader(new InputStreamReader(
	            		TextEdit.class.getResourceAsStream("test"), StandardCharsets.UTF_8));
	...
	private class WriteFileActionListener implements ActionListener {
		
		@Override
		//For the Handbook, first read contents from database and put those bytes into a file that we'd then read from here...?
		//If no Internet connection, then just read from the local file.
		//Perhaps the local files would need to be separate from the executable...
		public void actionPerformed(ActionEvent e) {
						
			//For Handbook, first write contents to local file and then update database?
			//If no Internet connection, don't update database...
			//Hmm, apparently resources are read-only, which means I won't be able to write to them...so
			//should I only update the database and if there's no Internet connection just say that update
			//cannot be made?
			
			DefaultStyledDocument doc = (DefaultStyledDocument) getEditorDocument();
			
			//HARD-CODING
			try (OutputStream fos = new FileOutputStream("src/test"); ObjectOutputStream oos = new ObjectOutputStream(fos)) {
			//try (OutputStream fos = new FileOutputStream(TextEdit.class.getResource("test").getPath()); 
			//		ObjectOutputStream oos = new ObjectOutputStream(fos)) {
					oos.writeObject(doc); //Writing to oos also writes to the FileOutputStream...
					oos.flush(); oos.close();
			}
		...
		public void actionPerformed(ActionEvent e) {
			try{
			    PrintWriter writer = new PrintWriter("src/test", "UTF-8");
			    writer.println(editor__.getText());
			    //writer.println(getEditorDocument());
	...
	private class ReadDbActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				// ===== CODE FOR GETTING A HARD-CODED ENTRY FROM DBConnect =====
				rs.next(); //Need to say this to get to the first row in the result table
								
				//InputStream --> ObjectInputStream
				//Think: ObjectInputStream deserializes primitive data and objects previously written using an ObjectOutputStream. 
				//ObjectInputStream ois = new ObjectInputStream(rs.getBlob("text").getBinaryStream());
				
				ObjectInputStream ois = new ObjectInputStream(rs.getBlob("content").getBinaryStream());
				...
				/*
				// ===== CODE FOR GETTING THE LAST ROW IN A RESULT TABLE =====
				rs.afterLast();
				while (rs.previous()) {
					ObjectInputStream ois = new ObjectInputStream(rs.getBlob("content").getBinaryStream());
					doc = (DefaultStyledDocument)ois.readObject();
					//System.out.println(rs.getInt("id"));
					break; //break so you bounce after only getting the last row
				}
				editor__.setDocument(doc);
				*/
	...
	private class UpdateDbActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try (ByteArrayOutputStream baos = new ByteArrayOutputStream(); ObjectOutputStream oos = new ObjectOutputStream(baos)) {
				...
				oos.writeObject(doc); //Writing to oos also writes to baos...
				oos.flush(); oos.close();
	...
	private class InsertIntoDbActionListener implements ActionListener {
	...
	private class SaveFileListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			//If the you have yet to save the content of the JTextPane, then the popup for saving your file
			//will appear. However, if you've already saved and are currently editing the file, then the save
			//will go though without the popup.
			if (file__ == null) {
			...
			setFrameTitleWithExtn(file__.getName()); 
			//Sets the title of the Frame everything is added onto to have the name of the current file
		}
